...
public class Measurement<F extends Field> {
  ...
  protected abstract static class Field {
    ...
    public static <F extends Field> F parse(final String key, final String input, final Class<F> clazz) {
      if (DoubleField.class.equals(clazz)) {
        return clazz.cast(new DoubleField(key, input));
      }
      else if (LongField.class.equals(clazz)) {
        return clazz.cast(new LongField(key, input));
      }
      else if (StringField.class.equals(clazz)) {
        return clazz.cast(new StringField(key, input));
      }
      else if (BooleanField.class.equals(clazz)) {
        return clazz.cast(new BooleanField(key, input));
      }
